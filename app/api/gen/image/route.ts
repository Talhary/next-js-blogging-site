export const POST = async(req:Request)=>{
    try {
        ...
        console.log(text,height,width,model)
        const res = await genDallE(text,height,width,model,count)
        console.log({res})
        return Response.json(res)
    } catch (error) {
        console.log(error)
        return Response.json({err:true,msg:error.message})
    }
}


const genDallE = async(text:string,height=1024,width=1024,model:string='dall-e-3',count=1) => {
